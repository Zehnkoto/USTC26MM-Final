...
PHYSICS_PANEL = SUPERSPLAT_ROOT / "src" / "ui" / "physics-panel.ts"
...
def main() -> None:
    ...
    assert_contains(backend, 'config["strength_scale"] = strength_value', "backend emits PBMPM strength scale")
    assert_contains(decode, '"n_min"', "decode PBMPM N min")
    assert_contains(decode, '"n_max"', "decode PBMPM N max")
    assert_contains(decode, '"strength_scale"', "decode PBMPM strength scale")
    ...
    assert_contains(gs, "strength_scale=pbmpm_params.get", "runtime passes PBMPM strength scale")
    ...
    if physics_session:
        ...
        assert_contains(physics_session, "strength_scale?: number", "frontend PBMPM strength type")
    physics_panel = read(PHYSICS_PANEL) if PHYSICS_PANEL.exists() else ""
    if physics_panel:
        assert_contains(physics_panel, "const pbmpmStrengthScale", "frontend PBMPM strength control")
        assert_contains(physics_panel, "strength_scale: pbmpmStrengthScale.value", "frontend emits PBMPM strength")
        assert_contains(physics_panel, "createParamRow('Strength'", "frontend shows PBMPM strength row")
