...
import { isPhysMotionManifest } from './phys-motion';
...
const isPhysMotionPackage = (filenames: string[]) => filenames.some(filename => isPhysMotionManifest(filename));
...
const initFileHandler = (scene: Scene, events: Events, dropTarget: HTMLElement) => {
    ...
    const importSplatModel = async (files: ImportFile[], animationFrame: boolean) => {
        try {
            ...
            const isDirectPly = mainFile.filename.toLowerCase().endsWith('.ply');
            // PhysGaussian motion indices are original PLY row ids. Keep direct
            // PLY imports in row order so positions, color, SH and opacity stay aligned.
            const skipReorder = !!mainFile.skipReorder || isDirectPly;
            const model = await scene.assetLoader.load(filename, fileSystem, animationFrame, skipReorder);
            await scene.add(model);
            if (mainFile.contents && mainFile.filename.toLowerCase().endsWith('.ply')) {
                void events.invoke('physics.syncImportedFile', mainFile);
            }
            return model;
        } catch (error) {
    ...
    const importFiles = async (files: ImportFile[], animationFrame = false) => {
        ...
        if (isPhysMotionPackage(filenames)) {
            const model = await events.invoke('physmotion.loadPackage', files) as Splat;
            if (model) result.push(model);
        } else if (isPlySequence(filenames)) {
            // handle ply sequence
            ...
            for (let i = 0; i < filenames.length; i++) {
                ...
                if (['.ssproj', '.ply', '.splat', '.sog', '.webp', 'images.txt', '.json', '.bin', '.ksplat', '.spz'].every(ext => !filename.endsWith(ext))) {
    ...
    if (!window.showOpenFilePicker) {
        ...
        fileSelector.setAttribute('accept', '.ply,.splat,meta.json,.json,.physmotion.json,.webp,.ssproj,.sog,.lcc,.bin,.txt,.ksplat,.spz');
