...
import { registerPhysMotionEvents } from './phys-motion';
import { registerPhysicsSessionEvents } from './physics-session';
...
const main = async () => {
    ...
    registerPhysicsSessionEvents(events);
    ...
    const requestedRenderer = url.searchParams.get('renderer')?.toLowerCase() ?? '';
    const webgpuAvailable = !!(navigator as any).gpu;
    const deviceTypes = requestedRenderer === 'webgpu' && webgpuAvailable
        ? ['webgpu', 'webgl2']
        : ['webgl2'];
    ...
    registerPhysMotionEvents(events, scene);
