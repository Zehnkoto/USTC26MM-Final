        ...
        const renderImpl = async () => {
            ...
            try {
                ...
                const prepareFrame = async (frameTime: number): Promise<Splat | null> => {
                    ...
                    // Wait for PLY sequence or lightweight PhysGaussian motion cache to load the frame if present.
                    const newSplat = await events.invoke('plysequence.setFrameAsync', Math.floor(frameTime)) as Splat | null;
                    await events.invoke('physmotion.setFrameAsync', Math.floor(frameTime));

                    // manually update the camera so position and rotation are correct
                    scene.camera.onUpdate(0);

                    // Full PLY sequences replace the splat resource, so they still need a fresh sort.
                    // Lightweight PhysGaussian motion frames update stable Gaussian indices in place;
                    // for interactive preview we keep the previous order and only sort when the camera moves.
                    if (newSplat) {
