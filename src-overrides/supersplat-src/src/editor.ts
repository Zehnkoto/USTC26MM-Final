...
import { SelectAllOp, SelectNoneOp, SelectInvertOp, SelectOp, HideSelectionOp, UnhideAllOp, DeleteSelectionOp, ResetOp, MultiOp, AddSplatOp, SelectionOp } from './edit-ops';
...
const registerEditorEvents = (events: Events, editHistory: EditHistory, scene: Scene) => {
    ...
    events.on('select.mask', (op: SelectionOp, mask: Uint8Array | Uint32Array) => {
        selectedSplats().forEach((splat) => {
            events.fire('edit.add', new SelectOp(splat, op, mask));
        });
    });

    const intersectCenters = (splat: Splat, op: SelectionOp, options: any) => {
    ...
    events.on('select.bySphere', async (op: SelectionOp, sphere: number[]) => {
    ...
    events.on('select.byBox', async (op: SelectionOp, box: number[]) => {
        for (const splat of selectedSplats()) {
            await intersectCenters(splat, op, {
    ...
    events.function('select.rect', async (op: SelectionOp, rect: any) => {
    ...
    events.function('select.point', async (op: SelectionOp, point: { x: number, y: number }) => {
        const { width, height } = scene.targetSize;
        const mode = events.invoke('camera.mode');
    ...
    events.function('select.pickIndex', async (point: { x: number, y: number, radiusPixels?: number }) => {
        ...
        const radiusPixels = Math.max(0, Math.min(12, Math.round(point.radiusPixels ?? 0)));
        const rectWidth = Math.max(1, radiusPixels * 2 + 1);
        const rectHeight = rectWidth;
        const rx = Math.max(0, Math.min(1, nx - radiusPixels / width));
        const ry = Math.max(0, Math.min(1, ny - radiusPixels / height));
        const rw = Math.min(1 - rx, rectWidth / width);
        const rh = Math.min(1 - ry, rectHeight / height);

        for (const splat of selectedSplats()) {
            scene.camera.pickPrep(splat, 'set');
            const pickBuffer = await scene.camera.pickRect(rx, ry, rw, rh);
            const pickId = pickBuffer?.find((id: number) => (
                id !== undefined &&
                id !== 0xffffffff &&
                id >= 0 &&
                id < splat.splatData.numSplats
            ));
            if (pickId !== undefined && pickId !== 0xffffffff && pickId >= 0 && pickId < splat.splatData.numSplats) {
                return {
                    index: pickId,
                    splat: splat.name ?? '',
    ...
    events.function('select.colorMatch', async (op: SelectionOp, point: { x: number, y: number }, threshold = 0) => {
    ...
    events.function('camera.getPose', () => {
        ...
        const right = camera.mainCamera.right;
        const up = camera.mainCamera.up;
